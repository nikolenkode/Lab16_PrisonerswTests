...
public class Main {
    private static final String TXT_PATH = "C:\\Users\\demyd\\OneDrive\\Desktop\\KHPI\\OOP\\PrisonersTests\\prisoners.txt";
    private static final String DAT_PATH = "C:\\Users\\demyd\\OneDrive\\Desktop\\KHPI\\OOP\\PrisonersTests\\prisoners.dat";
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case "8":
                    System.out.print("Введіть максимальний час виконання (таймаут) в мілісекундах: ");
                    long timeout = scanner.nextLong();
                    scanner.nextLine(); 

                    MyLinkedList<Prisoner> largeList = new MyLinkedList<>();
                    for (int i = 0; i < 15; i++) {
                        for (Prisoner p : list) {
                            largeList.add(p);
                        }
                    ...
                    System.out.println("\n--- Запуск 3-х паралельних потоків обробки ---");
                    long startTime = System.currentTimeMillis();

    Thread thread1 = new Thread(() -> {
        Prisoner tallest = PrisonerAnalyzer.findTallestPrisoner(largeList);
        if (tallest != null) {
            System.out.println("[Результат Потоку 1] Найвищий: " + tallest.getFullName());
        }
    });

    Thread thread2 = new Thread(() -> {
        double avg = PrisonerAnalyzer.calculateAverageHeight(largeList);
        if (avg > 0) {
            System.out.println("[Результат Потоку 2] Середній зріст: " + String.format("%.2f", avg) + " см");
        }
    });

    Thread thread3 = new Thread(() -> {
        int count = PrisonerAnalyzer.countByEyeColor(largeList, "green");
        if (count > 0 || !Thread.currentThread().isInterrupted()) {
            System.out.println("[Результат Потоку 3] Кількість з зеленими очима: " + count);
        }
    });
    ...
    if (thread3.isAlive()) thread3.interrupt();

    long estimatedTime = System.currentTimeMillis() - startTime;
    System.out.println("Загальний час виконання черги: " + estimatedTime + " мс.\n");
